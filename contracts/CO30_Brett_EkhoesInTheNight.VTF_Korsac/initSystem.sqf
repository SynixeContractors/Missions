...
mission_logic = {
    ...
    if ((currentWeapon vehicle ace_player select [0, 11]) == "hgun_esd_01") then {
...
addUserActionEventHandler ["nextweapon", "Activate", {
    if ((currentWeapon vehicle ace_player select [0, 11]) != "hgun_esd_01") exitWith {};
...
[{
    {
        ...
        private _seed = parseNumber ((netid _x) select [2,10]);
        private _delay = ((_seed random [0,0]) * 8) max 0.4;
        private _freq = (_seed random [1,1]) * 20 + 700;
        [_x, _delay, [[_freq, 150]]] call mission_delay;
    } forEach allUnits;
...
addMissionEventHandler ["Draw3D", {
    ...
    {
        private _pos = 2.5;
        private _loc = ASLToAGL getPosASL objectFromNetId _x;
        if (curatorCamera distance _loc > 600) then { continue };
        {
            drawIcon3D ["",[1,1,1,1],_loc,1,_pos,0,str (_x select 0),2,0.05,"PuristaMedium","center"];
            _pos = _pos + 1.5;
        } forEach _y;
