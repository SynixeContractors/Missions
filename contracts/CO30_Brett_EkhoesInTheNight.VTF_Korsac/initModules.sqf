    ...
    if (isNull(findDisplay 312)) exitWith {};
    {
        if !(_x getVariable ["synixe_spectator_dead", false]) then {
            if (_x getVariable ["wack_fucked_in_the_head", false]) then {
                drawIcon3D ["",[0.8,0,0,1],ASLToAGL getPosASL _x,1,2.5,0,"fucked",2,0.05,"PuristaMedium","center"];
            };
            if (_x getVariable ["wack_drugged", false]) then {
                drawIcon3D ["",[0.8,0,0,1],ASLToAGL getPosASL _x,1,4,0,"drugged",2,0.05,"PuristaMedium","center"];
            };
            if (_x getVariable ["wack_blind", false]) then {
                drawIcon3D ["",[0.8,0,0,1],ASLToAGL getPosASL _x,1,5.5,0,"blind",2,0.05,"PuristaMedium","center"];
            };
            ...
            if (_x getVariable ["wack_scramble", false]) then {
                drawIcon3D ["",[0.8,0,0,1],ASLToAGL getPosASL _x,1,10,0,"scramble",2,0.05,"PuristaMedium","center"];
            };
        };
    } forEach allPlayers;
    {
